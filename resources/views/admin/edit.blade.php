...
@section('body')
 ...
 <div class="card">
            ...
            <!-- /.card-header -->
            <div class="card-body">
              <form action="{{ route('admin.banner.update') }}" id="form_validation" method="POST" enctype="multipart/form-data">
                @csrf
    ...
    <input type="hidden" class="form-control-file" name="id" value="{{ $logos->id }}" id="exampleFormControlFile1">
     <img src="{{asset('/')}}{{$logos->banner}}" width="70px" height="70px">
  </div>

   <div class="form-group">
                        <label>Status</label>
                        <select class="form-control" name="status">
                          <option value="1"{{ 1 == $logos->status ? 'selected':'' }}>Active</option>
                          <option value="0"{{ 0 == $logos->status ? 'selected':'' }}>Inctive</option>
                      </select>
                      </div>

                
                  <button type="submit" class="btn btn-success">Update</button>
                   <a href="{{ route('admin.banner') }}" type="button" class="btn btn-danger">Cancel</a>
              </form>
            </div>
            <!-- /.card-body -->
          </div>
